...
export const analytics = {
  getRecoveryRate: (days: number = 30) =>
    api.get<{ recovery_rate: number; total_failures: number; recovered: number; period_days: number }>(
      `/v1/analytics/recovery_rate?days=${days}`
    ),
  getFailureCategories: () =>
    api.get<{ categories: Array<{ category: string; count: number; percentage: number }> }>(
      "/v1/analytics/failure_categories"
    ),
  getRevenueRecovered: (days: number = 30) =>
    api.get<{ total_recovered: number; currency: string; period_days: number }>(
      `/v1/analytics/revenue_recovered?days=${days}`
    ),
  getAttemptsByChannel: () =>
    api.get<{ channels: Array<{ channel: string; count: number; success_rate: number }> }>(
      "/v1/analytics/attempts_by_channel"
    ),
};
...
export const queryKeys = {
  ...
  analytics: {
    recoveryRate: (days: number) => ["analytics", "recovery-rate", days] as const,
    failureCategories: () => ["analytics", "failure-categories"] as const,
    revenueRecovered: (days: number) => ["analytics", "revenue-recovered", days] as const,
    attemptsByChannel: () => ["analytics", "attempts-by-channel"] as const,
  },
};
